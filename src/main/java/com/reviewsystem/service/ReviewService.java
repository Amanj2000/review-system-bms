...
public class ReviewService {
	...
	public ResponseDTO editReview(int movieId, String userEmail, ReviewRequestDTO reviewRequestDTO) {
		...
		movie.getReviews()
		     ...
		     .forEach(review -> review.setReview(reviewRequestDTO.getReview()));
		movieRepository.save(movie);

		return new ResponseDTO(String.format("review for movie %s updated successfully", movie.getMovieName()));
	}
	...
	public ResponseDTO deleteReview(int movieId, String userEmail) {
		...
		movieRepository.save(movie);
		return new ResponseDTO(String.format("review for movie %s deleted successfully", movie.getMovieName()));
	}
}
